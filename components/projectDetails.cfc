...
<cfcomponent displayname="projectDetails">
    <cffunction name="selectBug" access="public" output="false" returnType="query">
        <cfquery name="bugs" datasource="#Application.dataSourceName#">
            ...
            from bugs inner join status on status.statusID=bugs.statusID inner join priorities
            on priorities.priorityID=bugs.priorityID inner join severities
            on severities.severityID=bugs.severityID inner join projects
            on projects.projectID=bugs.projectID
        </cfquery>
        <cfreturn bugs/>
    </cffunction>
